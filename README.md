...
**Transfer moves a live session between daemons (Unix).** `cleat transfer ID --to NAME` hands the session's PTY, child process, and recording to another daemon on the same runtime root without interrupting it: the program keeps running, its screen is carried over, and the recording stays one file with a `transferred` marker. The session directory moves into the target's generation and the session's *hosting epoch* (`cleat inspect`) advances by one. Attached clients follow: `cleat attach` and `cleat watch` reconnect to the new daemon once and show a one-line notice, and library clients reopen their channel with the role they held. Session-ID commands find the session on its new daemon; the old daemon answers requests for it with a redirect for 30 seconds. If the target is unreachable, refuses the session, or is not ready within `--timeout` (10 seconds by default), nothing moves. A transfer that would strand attached clients on a daemon speaking an incompatible packet protocol is refused with a list of those clients unless `--drop-incompatible` is given. Holders can state the epoch they expect with `--hosting-epoch N`; mutating commands against a different epoch are refused as stale.
